...
public:
    select_view() requires std::default_initializable<R> = default;
    ...
    constexpr auto begin() requires std::ranges::range<R> {
    ...
    constexpr auto end() requires(std::ranges::range<R>) {
        return sentinel<false>(std::ranges::end(mBase));
    } 

    constexpr auto end() const requires(std::ranges::common_range<R>) {
    ...
    constexpr auto end() const requires(std::ranges::common_range<const R>) {
...
public:
    using iterator_concept = std::iterator_traits<base_iterator_t>::iterator_category;
    ...
    iterator() requires std::default_initializable<base_iterator_t> = default;
    ...
    requires std::ranges::random_access_range<base_range_t> && std::three_way_comparable<base_iterator_t>
    {
        return lhs.mCurrent <=> rhs.mCurrent;
    }
...
class select_view<R, N, Ns...>::sentinel {
    ...
    using base_iterator_t = std::ranges::iterator_t<std::conditional_t<OtherConst, const R, R>>;
...
public:
    ...
    requires Const && std::convertible_to<R, const R> : mEnd(move(other.end)) {}

    constexpr base_sentinel_t base() const { return mEnd; }

    template<bool OtherConst> requires std::sentinel_for<base_sentinel_t, base_iterator_t<OtherConst>>
    friend constexpr bool operator==(const iterator<OtherConst>& lhs, const sentinel& rhs) {
        return lhs.mCurrent == rhs.mEnd;
    }

    template<bool OtherConst> requires std::sized_sentinel_for<base_sentinel_t, base_iterator_t<OtherConst>>
    friend constexpr difference_type operator-(const iterator<OtherConst>& lhs, const sentinel& rhs) {
        return lhs.mCurrent - rhs.mEnd;
    }

    template<bool OtherConst> requires std::sized_sentinel_for<base_sentinel_t, base_iterator_t<OtherConst>>
    friend constexpr difference_type operator-(const sentinel& lhs, const iterator<OtherConst>& rhs) {
...
struct select_fn final {
    ...
    auto operator()(R&& range) const {
        return select_view<std::views::all_t<R&&>, I, Is...>(std::forward<R>(range));
    }
