...
class vector final {
 public:
  ...
  constexpr auto capacity() const noexcept -> std::size_t { return _capacity; }
  ...
  constexpr auto reserve(const std::size_t& capacity) -> void {
    ...
    std::byte* new_data = static_cast<std::byte*>(
        _allocator.allocate_bytes(new_capacity * packed_sizeof, max_alignof));
    if (_data) {
      [this]<std::size_t... Is>(std::byte* new_data,
                                const std::size_t& new_capacity,
                                std::index_sequence<Is...>) {
        return std::min({
            move_column<Is>(new_data, new_capacity, _data, _capacity, _size)...,
        });
 ...
 private:
  ...
  constexpr static auto column(const std::byte* data,
    ...
    return reinterpret_cast<
        const details::sequence_element_t<I, type_sequence>*>(
        data + capacity * details::packed_sizeof<
...
class vector<T, Ts...>::const_iterator : public details::index {
 public:
  ...
  using value_type = std::tuple<
      ...
      const details::sequence_element_t<Is, container_type::type_sequence>&...>;
  using reference = std::tuple<
      const details::sequence_element_t<I, container_type::type_sequence>&,
      const details::sequence_element_t<Is, container_type::type_sequence>&...>;
  using iterator_category = std::random_access_iterator_tag;
