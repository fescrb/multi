...
static void BM_select_iterate(benchmark::State& state) {
    ...
    for(std::size_t _ = 0; _ < state.range(0); ++_)
        v.push_back(val);
    std::tuple<std::tuple_element_t<0, typename Container::value_type>, std::tuple_element_t<2, typename Container::value_type>>  accum = {};
    for(auto _ : state) {
...
BENCHMARK(BM_single_select_iterate<std::vector<std::tuple<double, double, double, double>>>)->Range(8, std::pow(8,6));
BENCHMARK(BM_single_select_iterate<multi::vector<double, double, double, double>>)->Range(8, std::pow(8,6));
BENCHMARK(BM_select_iterate<std::vector<std::tuple<double, double, double, double>>>)->Range(8, std::pow(8,6));
BENCHMARK(BM_select_iterate<multi::vector<double, double, double, double>>)->Range(8, std::pow(8,6));